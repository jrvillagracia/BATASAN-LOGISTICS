...
$(document).ready(function() {
    ...
    $('#SuppliesSaveButton').on('click', function (e) {
        ...
        const otherCategory = $('#otherSuppCategory').val().trim();  // Get the value of "Other Category"
        ...
        if (controlNo === '' || brandName === '' || name === '' || finalCategory === '' || type === '' || color === '' || unit === '' || 
        ...
        $.ajax({
            ...
            success: function (response) {
                Swal.fire({
                    ...
                    // Append the new supplies data to the table
                    $('#tableBody').append(`
                        <tr class="odd:bg-blue-100 odd:dark:bg-gray-900 even:bg-white even:dark:bg-gray-800 border-b dark:border-gray-700" data-id="${response.suppliesId}">
                            ...
                            <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">${finalCategory}</td>  <!-- Show the final category -->
